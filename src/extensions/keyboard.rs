...
#[cfg(unix)]
fn read_key(call: &NativeCall<'_>, timeout_milliseconds: libc::c_int) -> Result<Value> {
    STATE.with(|state| {
        let mut state = state.borrow_mut();
        if !state.is_active() {
